...
- (void)open {
    MQTTLogDebug("[MQTTNWTransport] session");
...
#define EPHEMERAL 1
    ...
    MQTTLogDebug("[MQTTNWTransport] task");
    if (self.ws) {
    ...
    MQTTLogDebug("[MQTTNWTransport] resume");
    if (self.ws) {
...
- (void)read {
    MQTTLogDebug("[MQTTNWTransport] read");
    if (self.ws) {
        if (@available(iOS 13.0, macOS 10.15, *)) {
            [self.webSocketTask receiveMessageWithCompletionHandler:^(NSURLSessionWebSocketMessage * _Nullable message, NSError * _Nullable error) {
                MQTTLogDebug("[MQTTNWTransport] receiveMessage %@ %@ %ld",
        ...
        [self.streamTask readDataOfMinLength:0
         ...
         ^(NSData * _Nullable data, BOOL atEOF, NSError * _Nullable error) {
            MQTTLogDebug("[MQTTNWTransport] read %@ %d %@", data, atEOF, error);
            if (atEOF) {
                [self.delegate mqttTransportDidClose:self];
            } else if (error || !data) {
                [self.delegate mqttTransport:self didFailWithError:error];
...
- (BOOL)send:(NSData *)data {
    if (self.ws) {
        if (@available(iOS 13.0, macOS 10.15, *)) {
            MQTTLogDebug("[MQTTNWTransport] send ws %ld %@",
            ...
            [self.webSocketTask sendMessage:message
                ...
                MQTTLogDebug("[MQTTNWTransport] sendMessage error %@", error);
            }];
        } else {
            // Fallback on earlier versions
        }
    } else {
        MQTTLogDebug("[MQTTNWTransport] send stream %ld %@",
                     (long)self.streamTask.state,
                     self.streamTask.error);
        [self.streamTask writeData:data
                           timeout:0
                 completionHandler:^(NSError * _Nullable error) {
            MQTTLogDebug("[MQTTNWTransport] send error %@", error);
        }];
    }
    return TRUE;
}
 ...
 completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition, NSURLCredential * _Nullable))completionHandler {
    ...
    if (challenge.protectionSpace.authenticationMethod == NSURLAuthenticationMethodServerTrust) {
        ...
        for (CFIndex index = 0; index < certificateCount; index++) {
            ...
            MQTTLogDebug("[MQTTNWTransport] SecCertificateCopySubjectSummary %@", summary);
        }
        CFRelease(certs);
