...
- (NSData *)wireFormat {
    ...
    MQTTLogDebug("[MQTTMessage] wireFormat(%lu)=%@...",
    ...
    if (data.length >= 2) {
        ...
        do {
            if (data.length < offset) {
                MQTTLogError("[MQTTMessage] message data incomplete remaining length");
                offset = -1;
                break;
            }
            ...
            if (multiplier > 128*128*128*128) {
                MQTTLogError("[MQTTMessage] message data too long remaining length");
                multiplier = -1;
                break;
            }
        } while ((digit & 0x80) != 0);
        ...
        if (maximumPacketLength && remainingLength + offset > maximumPacketLength.unsignedLongValue) {
            MQTTLogError("[MQTTMessage] maximum Packet Size exceeded %u/%@",
            ...
            if (offset > 0 &&
                ...
                if ((type == MQTTPublish && (qos >= MQTTQosLevelAtMostOnce && qos <= MQTTQosLevelExactlyOnce)) ||
                    ...
                    if ((type == MQTTPublish &&
                        ...
                        if (message.data.length >= 2) {
                            ...
                            if (message.data.length >= 2 + topicLength + 2) {
                                ...
                                MQTTLogError("[MQTTMessage] missing packet identifier in PUBLISH");
                                message = nil;
                            }
                        } else {
                            MQTTLogError("[MQTTMessage] missing packet identifier");
                            message = nil;
                        }
                    }
                    if (type == MQTTPuback ||
                    ...
                    if (type == MQTTPuback ||
                        ...
                        if (protocolLevel != MQTTProtocolVersion50) {
                            if (message.data.length > 2) {
                                MQTTLogError("[MQTTMessage] unexpected payload after packet identifier");
                                message = nil;
                            }
                        } else {
                    ...
                    if (type == MQTTPingreq ||
                        ...
                        if (message.data.length > 0) {
                            MQTTLogError("[MQTTMessage] unexpected payload");
                            message = nil;
                        }
                    }
                    ...
                    if (type == MQTTConnect) {
                        if (message.data.length < 3) {
                            MQTTLogError("[MQTTMessage] missing connect variable header");
                            message = nil;
                        }
                    }
                    if (type == MQTTConnack) {
                        if (protocolLevel == MQTTProtocolVersion50) {
                            if (message.data.length < 3) {
                                MQTTLogError("[MQTTMessage] missing connack variable header");
                                message = nil;
                            }
                        } else {
                            if (message.data.length != 2) {
                    ...
                    if (type == MQTTSubscribe) {
                        if (message.data.length < 3) {
                            MQTTLogError("[MQTTMessage] missing subscribe variable header");
                            message = nil;
                        }
                    }
                    ...
                    if (type == MQTTSuback) {
                        if (protocolLevel != MQTTProtocolVersion50) {
                            if (message.data.length < 3) {
                                MQTTLogError("[MQTTMessage] missing suback variable header");
                                message = nil;
                            }
                        } else {
                            if (message.data.length < 4) {
                                MQTTLogError("[MQTTMessage] missing suback properties or variable header");
                                message = nil;
                            }
                        }
                    }
                    ...
                    if (type == MQTTUnsuback ) {
                        if (protocolLevel != MQTTProtocolVersion50) {
                            ...
                            if (message.data.length < 4) {
                                MQTTLogError("[MQTTMessage] missing unsuback properties or payload");
                                message = nil;
                            }
                        }
                    }
                    ...
                    if (type == MQTTUnsubscribe) {
                        if (message.data.length < 3) {
                            MQTTLogError("[MQTTMessage] missing unsubscribe variable header");
                            message = nil;
                        }
                    }
                } else {
                    MQTTLogError("[MQTTMessage] illegal header flags %02X", header);
                }
            } else {
                MQTTLogError("[MQTTMessage] remaining data wrong length");
            }
        } else {
            MQTTLogError("[MQTTMessage] illegal message type");
        }
    } else {
        MQTTLogError("[MQTTMessage] message data length < 2");
    }
    return message;
}
