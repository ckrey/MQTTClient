...
- (UInt16)subscribeToTopicsV5:(NSDictionary<NSString *,NSNumber *> *)topics
    ...
    MQTTLogDebug("[MQTTSession] subscribeToTopics:%@]", topics);
...
- (UInt16)unsubscribeTopicsV5:(NSArray<NSString *> *)topics
    ...
    MQTTLogDebug("[MQTTSession] unsubscribeTopicsV5:%@", topics);
...
- (UInt16)publishDataV5:(NSData *)data
 ...
 payloadFormatIndicator:(NSNumber *)payloadFormatIndicator
  messageExpiryInterval:(NSNumber *)messageExpiryInterval
    ...
    MQTTLogDebug("[MQTTSession] publishData:%@... onTopic:%@ retain:%d qos:%ld "
    ...
    if (!qos) {
        ...
        if (!flow) {
            MQTTLogDefault("[MQTTSession] dropping outgoing message %d", msgId);
            ...
            if ((flow.commandType).intValue == MQTTPublish) {
                MQTTLogDebug("[MQTTSession] PUBLISH %d", msgId);
                if (![self encode:msg]) {
                    MQTTLogInfo("[MQTTSession] queueing message %d after unsuccessfull attempt", msgId);
                    flow.commandType = [NSNumber numberWithUnsignedInt:MQTT_None];
                    flow.deadline = [NSDate date];
                    [self.persistence sync];
                }
            } else {
                MQTTLogInfo("[MQTTSession] queueing message %d", msgId);
            }
        }
    }
    [self tell];
...
- (void)closeWithReturnCode:(MQTTReturnCode)returnCode
    ...
    MQTTLogDebug("[MQTTSession] closeWithDisconnectHandler:%p ", disconnectHandler);
...
- (void)disconnectWithReturnCode:(MQTTReturnCode)returnCode
           sessionExpiryInterval:(NSNumber *)sessionExpiryInterval
                    reasonString:(NSString *)reasonString
                  userProperties:(NSArray <NSDictionary<NSString *,NSString *> *> *)userProperties {
    MQTTLogDebug("[MQTTSession] sending DISCONNECT");
...
- (void)closeInternal {
    MQTTLogDebug("[MQTTSession] closeInternal");
...
- (void)connackTimeout:(NSTimer *)timer {
    MQTTLogDebug("[MQTTSession] connackTimeout %@ @%.0f",
...
- (void)keepAlive:(NSTimer *)timer {
    MQTTLogDebug("[MQTTSession] keepAlive %@ @%.0f", self.clientId, [[NSDate date] timeIntervalSince1970]);
    (void)[self encode:[MQTTMessage pingreqMessage]];
}

- (void)checkDup:(NSTimer *)timer {
    MQTTLogDebug("[MQTTSession] checkDup %@ @%.0f", self.clientId, [[NSDate date] timeIntervalSince1970]);
    [self checkTxFlows];
}
            ...
            switch ((flow.commandType).intValue) {
                case MQTT_None:
                {
                    if (windowSize < self.persistence.maxWindowSize &&
                        (!self.brokerReceiveMaximum ||
                         windowSize < self.brokerReceiveMaximum.unsignedIntegerValue)) {
                        MQTTLogDebug("[MQTTSession] PUBLISH queued message %@", flow.messageId);
                ...
                case MQTTPublish:
                {
                    MQTTLogInfo("[MQTTSession] resend PUBLISH %@", flow.messageId);
                    NSString *topicToSend = [self topicToSend:flow.topic topicAlias:flow.topicAlias];
                    message = [MQTTMessage publishMessageWithData:flow.data
                                                          onTopic:topicToSend
                                                              qos:(flow.qosLevel).intValue
                ...
                case MQTTPubrel:
                    MQTTLogInfo("[MQTTSession] resend PUBREL %@", flow.messageId);
...
- (void)decoder:(MQTTDecoder*)sender handleEvent:(MQTTDecoderEvent)eventCode error:(NSError *)error {
    ...
    MQTTLogDebug("[MQTTSession] decoder handleEvent: %@ (%d) %@",
...
- (void)decoder:(MQTTDecoder*)sender didReceiveMessage:(NSData *)data {
    ...
    if (!message) {
        MQTTLogError("[MQTTSession] MQTT illegal message received");
        NSError * error = [NSError errorWithDomain:MQTTSessionErrorDomain
    ...
    @synchronized(sender) {
        ...
        switch (self.status) {
            ...
            case MQTTSessionStatusConnected:
                ...
                break;
            default:
                MQTTLogError("[MQTTSession] other state");
                break;
        }
    }
}
...
- (void)handlePublish:(MQTTMessage*)msg {
    ...
    if (!topic) {
        ...
        MQTTLogError("non UTF8 topic %@", topic);
    }
    ...
    if (msg.qos == 0) {
        ...
        if (msg.qos == 1) {
            ...
            if (self.protocolLevel == MQTTProtocolVersion50 &&
                ...
                MQTTLogDefault("[MQTTSession] receive maximum exceeded");
            ...
            if (![self.persistence storeMessageForClientId:self.clientId
                ...
                MQTTLogDefault("[MQTTSession] dropping incoming messages");
                if (self.protocolLevel == MQTTProtocolVersion50) {
                    (void)[self encode:[MQTTMessage disconnectMessage:MQTTProtocolVersion50
...
- (void)handlePubrec:(MQTTMessage *)message {
    MQTTLogDebug("handlePubrec mid:%d returnCode:%@", message.mid, message.returnCode);
...
- (UInt16)nextMsgId {
    MQTTLogDebug("nextMsgId synchronizing");
    @synchronized(self) {
        MQTTLogDebug("nextMsgId synchronized");
        self.txMsgId++;
        ...
        MQTTLogDebug("nextMsgId synchronized done");
        return self.txMsgId;
    }
}
...
- (void)connect {
    ...
    MQTTLogDebug("[MQTTSession] connecting");
...
- (void)connectWithConnectHandler:(MQTTConnectHandler)connectHandler {
    MQTTLogDebug("[MQTTSession] connectWithConnectHandler:%p", connectHandler);
...
- (BOOL)encode:(MQTTMessage *)message {
    if (message) {
        ...
        if (wireFormat) {
            ...
            MQTTLogDebug("[MQTTSession] mqttTransport send");
            return [self.transport send:wireFormat];
        } else {
            MQTTLogError("[MQTTSession] trying to send message without wire format");
            return false;
        }
    } else {
        MQTTLogError("[MQTTSession] trying to send nil message");
        return false;
    }
}
...
- (void)mqttTransport:(id<MQTTTransport>)mqttTransport didFailWithError:(NSError *)error {
    MQTTLogError("[MQTTSession] mqttTransport didFailWithError %@", error);
