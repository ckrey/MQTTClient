...
- (void)deleteAllFlowsForClientId:(NSString *)clientId {
    MQTTLogDebug("[MQTTCoreDataPersistence] deleteAllFlowsForClientId %@", clientId);
...
- (void)internalSync {
    if (self.managedObjectContext.hasChanges) {
        MQTTLogDebug("[MQTTPersistence] pre-sync: i%lu u%lu d%lu",
        ...
        if (![self.managedObjectContext save:&error]) {
            MQTTLogError("[MQTTPersistence] sync error %@", error);
        }
        if (self.managedObjectContext.hasChanges) {
            MQTTLogError("[MQTTPersistence] sync not complete");
        }
        MQTTLogDebug("[MQTTPersistence] postsync: i%lu u%lu d%lu",
                     (unsigned long)self.managedObjectContext.insertedObjects.count,
                     (unsigned long)self.managedObjectContext.updatedObjects.count,
                     (unsigned long)self.managedObjectContext.deletedObjects.count
                     );
...
- (NSArray *)allFlowsforClientId:(NSString *)clientId
    ...
    [self.managedObjectContext performBlockAndWait:^{
        ...
        if (!rows) {
            MQTTLogError("[MQTTPersistence] allFlowsforClientId %@", error);
        }
    }];
...
- (MQTTCoreDataFlow *)flowforClientId:(NSString *)clientId
    ...
    MQTTLogDebug("flowforClientId requestingPerform");
    [self.managedObjectContext performBlockAndWait:^{
        flow = [self internalFlowForClientId:clientId
                                incomingFlag:incomingFlag
                                   messageId:messageId];
    }];
    MQTTLogDebug("flowforClientId performed");
    return flow;
}
...
- (MQTTCoreDataFlow *)internalFlowForClientId:(NSString *)clientId
    ...
    MQTTLogDebug("flowforClientId performing");
    ...
    rows = [self.managedObjectContext executeFetchRequest:fetchRequest error:&error];
    if (!rows) {
        MQTTLogError("[MQTTPersistence] flowForClientId %@", error);
    } else {
        if (rows.count) {
...
- (NSPersistentStoreCoordinator *)createPersistentStoreCoordinator {
    ...
    MQTTLogDebug("[MQTTPersistence] Persistent store: %@", persistentStoreURL.path);
    ...
    if (![persistentStoreCoordinator addPersistentStoreWithType:self.persistent ? NSSQLiteStoreType : NSInMemoryStoreType
                                                  configuration:nil
                                                        ...
                                                        options:options
                                                          error:&error]) {
        MQTTLogError("[MQTTPersistence] managedObjectContext save: %@", error);
...
- (void)sizes {
    if (self.persistent) {
        ...
        MQTTLogDebug("[MQTTPersistence] Persistent store: %@", persistentStorePath);
    ...
    MQTTLogDebug("[MQTTPersistence] sizes %llu/%llu", self.fileSize, self.fileSystemFreeSize);
}
@end
