...
- (void)decodeMessage:(NSData *)data {
    ...
    while (self.state != MQTTDecoderStateInitializing &&
        ...
        if (self.state == MQTTDecoderStateDecodingHeader) {
            ...
            UInt8 header;
            MQTTLogDebug("[MQTTDecoder] header getBytes %ld/%lu",
            ...
            self.offset = 1;
            MQTTLogDebug("[MQTTDecoder] fixedHeader=0x%02x", header);
        }
        ...
        while (self.state == MQTTDecoderStateDecodingLength) {
            ...
            UInt8 digit;
                MQTTLogDebug("[MQTTDecoder] length getBytes %ld/%lu",
                           (long)readHere, (unsigned long)data.length);
            [data getBytes:&digit range:NSMakeRange(readHere, 1)];
            readHere++;
            MQTTLogDebug("[MQTTDecoder] digit=0x%02x 0x%02x %d %d",
            ...
            if ((digit & 0x80) == 0x00) {
                ...
                MQTTLogDebug("[MQTTDecoder] remainingLength=%d", (unsigned int)self.length);
            } else {
        ...
        if (self.state == MQTTDecoderStateDecodingData) {
            if (self.length > 0) {
                ...
                if (toRead > 0) {
                    MQTTLogDebug("[MQTTDecoder] buffer getBytes %ld/%lu %ld", (long)readHere, (unsigned long)data.length, (long)toRead);
            ...
            if (self.dataBuffer.length == self.length + self.offset) {
                MQTTLogDebug("[MQTTDecoder] received (%lu)=%@...", (unsigned long)self.dataBuffer.length,
                ...
                MQTTLogDebug("[MQTTDecoder] oops received (%lu)=%@...",
