...
- (void)test_tA_available {
    ...
    MQTTLogDefault("Server Topic Alias Maximum: %ld", self.session.brokerTopicAliasMaximum.integerValue);
...
- (void)test_any:(NSInteger)count topics:(NSInteger)topics sendTA:(BOOL)sendTA {
    ...
    for (NSInteger t = 1; t <= topics; t++) {
        for (NSInteger c = 1; c <= count; c++) {
            MQTTLogDefault("Publishing %ld", (long)c);
            done = false;
    ...
    while (!self.timedout) {
        MQTTLogDefault("Waiting for newMessages %ld/%ld",
