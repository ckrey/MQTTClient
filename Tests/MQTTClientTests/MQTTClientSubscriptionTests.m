...
- (void)testSubscribe_with_wrong_flags_MQTT_3_8_1_1 {
    ...
    MQTTLogDebug("can't test [MQTT-3.8.1-1]");
}

- (void)testUnsubscribe_with_wrong_flags_MQTT_3_10_1_1 {
    MQTTLogDebug("can't test [MQTT-3.10.1-1]");
}
...
- (void)testSubscribeWMultipleTopics_16_to_256 {
    ...
    for (int TOPICS = 16; TOPICS <= 256; TOPICS += 16) {
        ...
        MQTTLogDebug("testing %d subscriptions", TOPICS);
...
- (void)testSubscribeTopicHashnotalone_MQTT_4_7_1_2 {
    ...
    MQTTLogDebug("subscribing to topic %@", topic);
...
- (void)testSubscribeLong_MQTT_4_7_3_3 {
    ...
    MQTTLogDebug("LongSubscribe (%lu)", strlen([[topic substringFromIndex:1] UTF8String]));
    ...
    MQTTLogDebug("LongSubscribe (%lu)", strlen([[topic substringFromIndex:15] UTF8String]));
    ...
    MQTTLogDebug("LongSubscribe (%lu)", strlen([[topic substringFromIndex:2] UTF8String]));
...
- (void)test_very_long {
    ...
    NSMutableData *data = [[@"a" dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:FALSE] mutableCopy];
    
    NSInteger sent_messages = 0;
    
    while (data.length <= 1024L * 1024L * 2L - 1) {
    ...
    [self performSelector:@selector(timedout:)
               withObject:nil
               afterDelay:self.timeoutValue];
    while (self.newMessages != sent_messages && !self.timedout) {
        [[NSRunLoop currentRunLoop] runUntilDate:[NSDate dateWithTimeIntervalSinceNow:1]];
    }
    XCTAssert(self.newMessages == sent_messages,
              @"%ld/%ld Very long messages not received",
              self.newMessages, sent_messages);
...
- (void)testSubscribe:(NSString *)topic atLevel:(UInt8)qos {
    ...
    MQTTLogDebug("sent mid(SUBSCRIBE): %d", self.sentSubMid);
    [self performSelector:@selector(timedout:)
               withObject:nil
               afterDelay:self.timeoutValue];
...
- (void)testMultiSubscribe:(NSDictionary *)topics {
    ...
    MQTTLogDebug("sent mid(SUBSCRIBE multi): %d", self.sentSubMid);
    [self performSelector:@selector(timedout:)
               withObject:nil
               afterDelay:self.timeoutValue];
...
- (void)testUnsubscribeTopic:(NSString *)topic {
    ...
    MQTTLogDebug("sent mid(UNSUBSCRIBE): %d", self.sentUnsubMid);
...
- (void)testMultiUnsubscribeTopic:(NSArray *)topics {
    ...
    self.sentUnsubMid = [self.session unsubscribeTopicsV5:topics
                                           userProperties:nil
                                       unsubscribeHandler:nil];
    MQTTLogDebug("sent mid(UNSUBSCRIBE multi): %d", self.sentUnsubMid);
    [self performSelector:@selector(timedout:)
               withObject:nil
               afterDelay:self.timeoutValue];
