...
- (void)testPublish_r0_q0_0xFEFF_MQTT_1_5_3_3 {
    ...
    //                        atLevel:0];
    // [self shutdown];
    MQTTLogDefault("Can't test[MQTT-1.5.3-3]");
}
...
- (void)testPublish_r0_q0_0xD800_MQTT_1_5_3_1 {
    ...
    MQTTLogDebug("stringWithD800(%lu) %@", (unsigned long)stringWithD800.length, stringWithD800.description);
...
- (void)testPublish_r0_q0_0x9c_MQTT_1_5_3_1 {
    ...
    // MQTTLogDebug("stringWith9c(%lu) %@", (unsigned long)stringWith9c.length, stringWith9c.description);
    // [self connect];
    // [self testPublishCloseExpected:[@(__FUNCTION__) dataUsingEncoding:NSUTF8StringEncoding]
    ...
    // [self shutdown];
    MQTTLogDefault("Can't test[MQTT-1.5.3-1]");
}
...
- (void)testPublish_r0_q0_0x0000_MQTT_1_5_3_2 {
    ...
    MQTTLogDebug("stringWithNull(%lu) %@",
...
- (void)publish_illegal_topic_strict:(NSString *)topic {
    ...
    @try {
        ...
        MQTTLogDefault("Exception correctly raised: %@", exception);
    } @finally {
        [self shutdown];
    }
}
...
- (void)testPublish_a_lot_of_q0_retained {
    self.session.clientId = [NSString stringWithFormat:@"%s", __FUNCTION__];
    [self connect];
    ...
    for (int i = 0; i < ALOT; i++) {
        ...
        self.sentMessageMid = [self.session publishDataV5:data
                               ...
                               ^(NSError * _Nullable error,
                                 ...
                                 UInt16 msgID) {
            MQTTLogDefault("publishHandler %@ %@ %@ %@ %u",error, reasonString, userProperties, reasonCode, msgID);
            deliveredMessages++;
        }];
        MQTTLogDefault("testing publish %d/%d", i, self.sentMessageMid);
        publishedMessages++;
    }
    self.timedout = false;
    self.event = -1;
    ...
    while ((publishedMessages != self.deliveredMessages ||
           ...
           !self.timedout &&
           self.event == -1) {
        MQTTLogDefault("[MQTTClientPublishTests] waiting for %lu/%lu/%lu",
    ...
    for (int i = 0; i < ALOT; i++) {
        ...
        MQTTLogDefault("testing publish null %d/%d", i, self.sentMessageMid);
        publishedMessages++;
    }
...
- (void)testPublish_a_lot_of_q1_retained {
    self.session.clientId = [NSString stringWithFormat:@"%s", __FUNCTION__];
    [self connect];
...
- (void)testPublish_r1_q2_long_topic {
    ...
    while (topic.length <= 32768) {
        MQTTLogDebug("LongPublishTopic (%lu)", topic.length);
        [self testPublish:[@(__FUNCTION__) dataUsingEncoding:NSUTF8StringEncoding]
...
- (void)testPublish_r1_q2_long_payload {
    ...
    while (strlen([payload substringFromIndex:1].UTF8String) <= 1000000) {
        MQTTLogDebug("LongPublishPayload (%lu)", strlen([[payload substringFromIndex:1] UTF8String]));
...
- (void)testPublishNoUTF8_MQTT_3_3_2_1 {
    MQTTLogDefault("Can't test[MQTT-3.3.2-1]");
}
...
- (void)testPublishWithPlus_MQTT_3_3_2_2 {
    ...
    MQTTLogDebug("publishing to topic:%@", topic);
...
- (void)testPublishCloseExpected:(NSData *)data
    ...
    while (!self.timedout && self.event == -1) {
        MQTTLogDebug("[MQTTClientPublishTests] waiting for event %ld", (long)self.event);
        [[NSRunLoop currentRunLoop] runUntilDate:[NSDate dateWithTimeIntervalSinceNow:1]];
    }
    [NSObject cancelPreviousPerformRequestsWithTarget:self];

    MQTTLogDebug("testPublishCloseExpected event:%ld", (long)self.event);
    XCTAssert(
...
- (void)testPublishCore:(NSData *)data
    ...
    while (self.deliveredMessageMid != self.sentMessageMid && !self.timedout && self.event == -1) {
        MQTTLogDebug("[MQTTClientPublishTests] waiting for %d", self.sentMessageMid);
        [[NSRunLoop currentRunLoop] runUntilDate:[NSDate dateWithTimeIntervalSinceNow:1]];
    }
    [NSObject cancelPreviousPerformRequestsWithTarget:self];
}
...
- (BOOL)ignoreReceived:(MQTTSession *)session
              ...
              retained:(BOOL)retained
                 duped:(BOOL)duped
                   mid:(UInt16)mid
                data:(NSData *)data {
    MQTTLogDebug("ignoreReceived:%d qos:%d retained:%d duped:%d mid:%d data:%@", type, qos, retained, duped, mid, data);
...
- (void)messageDeliveredV5:(MQTTSession *)session
    ...
    MQTTLogDefault("messageDelivered %d", msgID);
...
subscriptionIdentifiers:(NSArray<NSNumber *> *)subscriptionIdentifiers {
    MQTTLogDefault("newMessage %d", mid);
