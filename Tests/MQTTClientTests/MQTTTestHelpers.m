...
- (void)ticker:(NSTimer *)timer {
    MQTTLogDebug("[MQTTTestHelpers] ticker");
}

- (void)timedout:(id)object {
    MQTTLogDebug("[MQTTTestHelpers] timedout");
    self.timedout = TRUE;
}
...
- (void)messageDeliveredV5:(MQTTSession *)session
    ...
    MQTTLogDebug("[MQTTTestHelpers] messageDeliveredV5 %d q%d r%d pfa=%@ mei=%@ ta=%@ rt=%@ cd=%@ up=%@ ct=%@ %@:%@",
                 msgID,
                 qos,
                 retainFlag,
...
subscriptionIdentifiers:(NSArray<NSNumber *> *)subscriptionIdentifiers {
    MQTTLogDebug("[MQTTTestHelpers] newMessageV5 %d q%d r%d pfa=%@ mei=%@ ta=%@ rt=%@ cd=%@ up=%@ ct=%@ si=%@ %@:(%ld)%@",
                 mid,
                 qos,
                 retained,
                 payloadFormatIndicator,
...
- (void)handleEvent:(MQTTSession *)session event:(MQTTSessionEvent)eventCode error:(NSError *)error {
    MQTTLogDebug("[MQTTTestHelpers] handleEvent:%ld error:%@", (long)eventCode, error);
    if (self.event == -1)  {
...
- (void)protocolError:(MQTTSession *)session error:(NSError *)error {
    MQTTLogDebug("[MQTTTestHelpers] protocolError: %@", error);
    if (self.event == -1)  {
...
- (void)connected:(MQTTSession *)session sessionPresent:(BOOL)sessionPresent {
    ...
    MQTTLogDebug("[MQTTTestHelpers] connected to %@:%d, "
                 "sP:%d, "
                 "bMPS:%@, "
                 "kA:%d, "
                 "sKA:%@, "
...
- (void)sending:(MQTTSession *)session
       ...
       retained:(BOOL)retained
          duped:(BOOL)duped
            mid:(UInt16)mid
           data:(NSData *)data {
    MQTTLogDebug("[MQTTTestHelpers] sending: %02X q%d r%d d%d m%d (%ld) %@",
...
- (void)received:(MQTTSession *)session
        ...
        retained:(BOOL)retained
           duped:(BOOL)duped
             mid:(UInt16)mid
            data:(NSData *)data {
    MQTTLogDebug("[MQTTTestHelpers] received:%d qos:%d retained:%d duped:%d mid:%d (%ld) data:%@",
                 type, qos, retained, duped, mid, data.length,
                 data.length < 64 ? data.description : [data subdataWithRange:NSMakeRange(0, 64)].description);
    self.type = type;
}
...
- (void)subAckReceivedV5:(MQTTSession *)session
    ...
    MQTTLogDebug("[MQTTTestHelpers] subAckReceivedV5 m%d rs=%@ up=%@ rc=%@",
                 msgID,
                 reasonString,
                 userProperties,
                 reasonCodes);
...
- (void)unsubAckReceivedV5:(MQTTSession *)session
            ...
            userProperties:(NSArray<NSDictionary<NSString *,NSString *> *> *)userProperties
               reasonCodes:(NSArray<NSNumber *> *)reasonCodes {
    MQTTLogDebug("[MQTTTestHelpers] unsubAckReceivedV5 m%d rs=%@ up=%@ rc=%@",
                 msgID,
                 reasonString,
                 userProperties,
                 reasonCodes);
...
- (NSArray *)clientCerts {
    ...
    if (self.parameters[@"clientp12"] && self.parameters[@"clientp12pass"]) {
        ...
        if (!clientCerts) {
            MQTTLogDebug("[MQTTTestHelpers] invalid p12 file");
        }
    }
    return clientCerts;
}
